...
class View(writeable):
    ...
    def setSuggestionsTrue(self):
        ''' Sets the view to have suggestions: yes'''
        self.properties.addProperty('suggestions','yes')
        return self    
...
class Field(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.identifier = kwargs.get('idenimtifier', None)
        if not self.identifier:
            self.identifier = kwargs.get('name', None)
        if not self.identifier:
            if len(args) > 1:
